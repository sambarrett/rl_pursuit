...
void World::step(std::vector<ActionProbs> &actionProbList) {
  step(boost::shared_ptr<std::vector<Action::Type> >(),actionProbList);
}
...
void World::storePositions() {
  world->generateObservation(storedObs,false);
}

void World::loadPositions() {
  world->setPositionsFromObservation(storedObs);
}

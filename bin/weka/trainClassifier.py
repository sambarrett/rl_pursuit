...
def getSaveFilename(options):
  if options.save:
    if options.name.startswith('twostagetransfer'):
      return os.path.join('configs','learners','saved','twostagetransfer','target%i-source%i' % (options.numTarget,options.numSource),'%s.txt' % options.student)
    else:
